...
def enviar_email_confirmacion_cita(cita):
    ...
    if cita.precio_cobrado:
        try:
            # Convertir a float para formatear correctamente
            precio_float = float(cita.precio_cobrado)
            # Si es un número entero, mostrar sin decimales
            if precio_float == int(precio_float):
                precio_texto = f"${precio_float:,.0f}".replace(',', '.')
            else:
                # Si tiene decimales, mostrar con 2 decimales
                precio_texto = f"${precio_float:,.2f}".replace(',', '.')
        except (ValueError, TypeError):
            # Si hay error al convertir, mostrar el valor original
            precio_texto = f"${cita.precio_cobrado}"

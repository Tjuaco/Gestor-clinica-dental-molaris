...
from .models import Cita, TipoServicio, HorarioDentista, Sala
...
# Notificaciones ahora se envían a través de mensajeria_service (correo electrónico)
...
class TrabajadorLoginView(LoginView):
    ...
    def form_invalid(self, form):
        ...
        # Nota: En producción, considerar usar un middleware de rate limiting más robusto
        if not settings.DEBUG:  # Solo en producción para no afectar desarrollo
...
def agregar_hora(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Enviar notificaciones por correo electrónico si la cita está reservada
...
def cancelar_cita_admin(request, cita_id):
    ...
    if cita.cancelar():
        # Enviar notificaciones de cancelación por correo electrónico
        try:
            ...
            if resultado.get('email', {}).get('enviado'):
                ...
                error_email = resultado.get('email', {}).get('error', 'Error desconocido')
                logger.warning(f"No se pudo enviar correo de cancelación para cita {cita.id}: {error_email}")
        except Exception as e:
            import logging
            logger = logging.getLogger(__name__)
...
def marcar_no_show(request, cita_id):
    ...
    # Solo se puede marcar como "No Show" (completada sin ingreso) si está en estado reservada o confirmada
    ...
    # Marcar como completada (sin ingreso) y guardar motivo
    cita.estado = 'completada'
    cita.motivo_no_asistencia = motivo_no_asistencia
    cita.precio_cobrado = Decimal('0.00')  # No se cobra nada
    cita.completada_por = perfil  # Registrar quién la marcó
    cita.fecha_completada = timezone.now()  # Registrar fecha de completado
    cita.save()
    
    mensaje = f'Cita marcada como "No Show" (completada sin ingreso) para {cita.paciente_nombre or "Sin nombre"} ({cita.fecha_hora.strftime("%d/%m/%Y %H:%M")}). Motivo registrado en el historial.'
...
def marcar_listo_para_atender(request, cita_id):
    ...
    cita = get_object_or_404(Cita.objects.select_related('dentista', 'cliente', 'tipo_servicio'), id=cita_id)
...
def iniciar_atencion(request, cita_id):
    ...
    # Registrar auditoría - Iniciar atención
    registrar_auditoria(
        usuario=perfil,
        accion='cambio_estado',
        modulo='citas',
        descripcion=f'Atención iniciada para paciente',
        detalles=f'Cita ID: {cita.id}, Paciente: {cita.paciente_nombre or (cita.cliente.nombre_completo if cita.cliente else "N/A")}, Estado anterior: {estado_anterior}, Estado nuevo: en_progreso',
        objeto_id=cita.id,
        tipo_objeto='Cita',
        request=request
    )
...
def finalizar_atencion(request, cita_id):
    ...
    # Registrar auditoría - Finalizar atención
    registrar_auditoria(
        usuario=perfil,
        accion='cambio_estado',
        modulo='citas',
        descripcion=f'Atención finalizada para paciente',
        detalles=f'Cita ID: {cita.id}, Paciente: {cita.paciente_nombre or (cita.cliente.nombre_completo if cita.cliente else "N/A")}, Estado anterior: {estado_anterior}, Estado nuevo: finalizada',
        objeto_id=cita.id,
        tipo_objeto='Cita',
        request=request
    )
...
def completar_cita_recepcion(request, cita_id):
    ...
    # Se puede completar si está en estado "finalizada" o editar precio si ya está "completada"
    es_edicion = cita.estado == 'completada'
    puede_completar = cita.estado == 'finalizada'
    
    if not puede_completar and not es_edicion:
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            from django.http import JsonResponse
            return JsonResponse({'success': False, 'error': f'La cita debe estar en estado "Finalizada" para completarla o "Completada" para editar el precio. Estado actual: "{cita.get_estado_display()}".'}, status=400)
        messages.error(request, f'La cita debe estar en estado "Finalizada" o "Completada". Estado actual: "{cita.get_estado_display()}".')
        return redirect('panel_trabajador')
    ...
    precio_anterior = cita.precio_cobrado
    
    # Cambiar estado solo si no está completada
    if not es_edicion:
        cita.estado = 'completada'
    ...
    if es_edicion:
        registrar_auditoria(
            ...
            accion='editar',
            modulo='citas',
            descripcion=f'Precio de cita actualizado - Precio anterior: ${precio_anterior:,.0f}, Precio nuevo: ${cita.precio_cobrado:,.0f}',
            detalles=f'Cita ID: {cita.id}, Precio anterior: ${precio_anterior:,.0f}, Precio nuevo: ${cita.precio_cobrado:,.0f}',
            objeto_id=cita.id,
            tipo_objeto='Cita',
            request=request
        )
        mensaje = f'Precio de la cita actualizado exitosamente. Precio cobrado: ${cita.precio_cobrado:,.0f}'
    else:
        registrar_auditoria(
            usuario=perfil,
            accion='cambio_estado',
...
def marcar_llegada(request, cita_id):
    ...
    # Se puede marcar llegada en cualquier estado excepto los finales (completada, cancelada)
    ...
    estados_no_permitidos = ['completada', 'cancelada', 'disponible']
...
def marcar_no_llego(request, cita_id):
    ...
    Alias de marcar_no_show para mantener compatibilidad
    SOLO DISPONIBLE PARA ADMINISTRATIVOS
    """
    # Redirigir a marcar_no_show para evitar duplicación
    return marcar_no_show(request, cita_id)
...
def completar_cita(request, cita_id):
    ...
    # Verificar que la cita esté en estado "finalizada" (según el flujo: después de crear ficha)
    if cita.estado != 'finalizada':
        messages.error(request, f'Esta cita está en estado "{cita.get_estado_display()}" y no puede ser completada. La cita debe estar "Finalizada" (después de crear la ficha odontológica).')
        return redirect('panel_trabajador')
...
def citas_dia(request):
    ...
    # Usar función helper para obtener citas filtradas
    from .helpers_citas import (
        obtener_citas_filtradas, agregar_info_fichas, 
        obtener_estadisticas_citas, obtener_contexto_base_citas, paginar_citas
    )
    ...
    citas_hoy = obtener_citas_filtradas(
        estado=['reservada', 'confirmada', 'en_espera', 'listo_para_atender', 'en_progreso', 'finalizada', 'disponible'],
        search_query=search_query,
        fecha=fecha_hoy,
        exclude_canceladas=True
    ).order_by('fecha_hora')
    
    # Agregar información de fichas
    citas_hoy_list = agregar_info_fichas(citas_hoy)
    
    # Paginación
    citas_pag = paginar_citas(citas_hoy_list, page)
    
    # Estadísticas
    estadisticas = obtener_estadisticas_citas(fecha_hoy)
    
    # Contexto base
    contexto_base = obtener_contexto_base_citas(perfil)
    
    # Citas pasadas para sidebar
...
def citas_disponibles(request):
    ...
    # Obtener citas usando función helper
    citas_list = obtener_citas_filtradas(
        estado='disponible',
        search_query=search_query
    ).order_by('fecha_hora')
    
    # Agregar información de fichas
    citas_list_processed = agregar_info_fichas(citas_list)
    
    # Paginación
    citas_pag = paginar_citas(citas_list_processed, page)
...
def citas_tomadas(request):
    ...
    # Paginación
    citas = paginar_citas(citas_list_processed, page)
...
def citas_completadas(request):
    ...
    # Citas completadas incluyen 'completada' y 'cancelada' (todas son parte del historial)
    # Nota: Las citas que no llegaron ahora se marcan como 'completada' con precio_cobrado=0
    citas_list = obtener_citas_filtradas(
...
def calendario_citas(request):
    ...
    # Estadísticas
    # El contador debe excluir las citas canceladas para ser consistente
    estadisticas = {
        'citas_hoy': Cita.objects.filter(fecha_hora__date=timezone.now().date()).exclude(estado='cancelada').count(),
...
def dashboard(request):
    ...
    if perfil.es_administrativo():
        ...
        # Citas
        # Excluir citas canceladas para que el contador coincida con la lista
        citas_hoy = Cita.objects.filter(fecha_hora__date=hoy).exclude(estado='cancelada').count()
        ...
        # Citas de hoy con detalles (excluir canceladas para consistencia)
        citas_hoy_detalle = Cita.objects.filter(
            fecha_hora__date=hoy
        ).exclude(estado='cancelada').select_related('dentista', 'cliente').order_by('fecha_hora')
        ...
        # Excluir citas canceladas para que el contador coincida con la lista
        citas_hoy = Cita.objects.filter(fecha_hora__date=hoy, dentista=perfil).exclude(estado='cancelada').count()
        ...
        # Citas de hoy del dentista (excluir canceladas para consistencia)
        citas_hoy_detalle = Cita.objects.filter(
            fecha_hora__date=hoy,
            dentista=perfil
        ).exclude(estado='cancelada').select_related('cliente').order_by('fecha_hora')
...
def dashboard_dentista(request):
    ...
    # Citas próximas (próximas 7 días) - optimizado
    citas_proximas = Cita.objects.filter(
        dentista=perfil,
        fecha_hora__date__gte=hoy,
        fecha_hora__date__lte=proximos_7_dias,
    ...
    # Contar pacientes únicos del dentista (optimizado: solo contar clientes no nulos)
    pacientes_count = Cita.objects.filter(
        dentista=perfil,
    ...
    if request.method == 'POST':
        ...
        if errores:
            ...
            try:
                ...
                logger.error(f'Error al agregar personal: {error_detail}\n{error_traceback}')
...
def mis_citas_dentista(request):
    ...
    ).select_related('cliente', 'dentista', 'tipo_servicio', 'plan_tratamiento')
    ...
    # Obtener IDs de todas las citas para optimizar consulta de odontogramas
    citas_ids_activas = list(citas_activas.values_list('id', flat=True))
    citas_ids_completadas = list(citas_completadas.values_list('id', flat=True))
    todas_citas_ids = citas_ids_activas + citas_ids_completadas
    
    # Obtener odontogramas de todas las citas en una sola consulta (optimizado)
    if todas_citas_ids:
        odontogramas_dict = {
            od.cita_id: od for od in Odontograma.objects.filter(
                cita_id__in=todas_citas_ids
            ).select_related('cita')
        }
    ...
    # Función helper para procesar citas (elimina código duplicado)
    def procesar_citas(citas_queryset):
        citas_list = list(citas_queryset)
        
        # Obtener emails únicos de citas sin cliente para vincular en batch
        emails_sin_cliente = {
            cita.paciente_email for cita in citas_list 
            if not cita.cliente and cita.paciente_email
        }
        
        # Vincular clientes en batch (más eficiente)
        if emails_sin_cliente:
            clientes_dict = {
                cliente.email: cliente 
                for cliente in Cliente.objects.filter(
                    email__in=emails_sin_cliente, 
                    activo=True
                )
            }
            ...
            for cita in citas_list:
                if not cita.cliente and cita.paciente_email:
                    cita.cliente = clientes_dict.get(cita.paciente_email)
            ...
            # Si no hay emails para vincular, solo marcar fichas
            for cita in citas_list:
                cita.tiene_ficha = cita.id in citas_con_ficha
                if cita.tiene_ficha:
                    cita.odontograma = odontogramas_dict.get(cita.id)
    ...
    # Procesar ambas listas usando la función helper
    citas_activas_list = procesar_citas(citas_activas)
    citas_completadas_list = procesar_citas(citas_completadas)
    
    # Calcular estadísticas solo para citas del dentista (optimizado)
    ahora = timezone.now()
    hoy = ahora.date()
    citas_dentista = Cita.objects.filter(dentista=perfil)
    total_citas = citas_dentista.count()
    citas_hoy = citas_dentista.filter(fecha_hora__date=hoy).count()
...
# ELIMINADO: tomar_cita_dentista - Los dentistas ya no pueden tomar citas directamente
# Solo los administrativos pueden asignar citas. Si se necesita esta funcionalidad,
# debe implementarse desde el panel administrativo.
    ...
    if request.method == 'POST':
        ...
        if errores:
            ...
            try:
                ...
                # Registrar auditoría - Crear odontograma
                registrar_auditoria(
                    usuario=perfil,
                    accion='crear',
                    modulo='odontogramas',
                    descripcion=f'Ficha odontológica creada para {paciente_nombre}',
                    detalles=f'Cita ID: {cita_obj.id}, Paciente: {paciente_nombre}, Motivo: {motivo_consulta[:100] if motivo_consulta else "N/A"}',
                    objeto_id=odontograma.id,
                    tipo_objeto='Odontograma',
                    request=request
                )
                
                # Crear documento asociado automáticamente
                try:
                    cliente_doc = cliente_obj
                    if not cliente_doc and paciente_email:
                        try:
                            cliente_doc = Cliente.objects.get(email=paciente_email, activo=True)
                        except Cliente.DoesNotExist:
                            cliente_doc = None
                        except Cliente.MultipleObjectsReturned:
                            cliente_doc = Cliente.objects.filter(email=paciente_email, activo=True).first()
                    ...
                    DocumentoCliente.objects.get_or_create(
                        ...
                        defaults={
                            'cliente': cliente_doc,
                            'titulo': f'Ficha Odontológica - {paciente_nombre}',
                            'descripcion': f'Ficha odontológica del paciente {paciente_nombre}',
                            'generado_por': perfil,
                ...
                if cita_obj.estado in ['en_progreso', 'listo_para_atender']:
                    ...
                    # Registrar auditoría - Cambio de estado de cita
                    registrar_auditoria(
                        usuario=perfil,
                        accion='cambio_estado',
                        modulo='citas',
                        descripcion=f'Cita marcada como finalizada al crear ficha odontológica',
                        detalles=f'Cita ID: {cita_obj.id}, Estado anterior: {cita_obj.estado}, Estado nuevo: finalizada',
                        objeto_id=cita_obj.id,
                        tipo_objeto='Cita',
                        request=request
                    )
                ...
                # Si viene desde una cita (tiene cita_id), redirigir a mis_citas
                if cita_id and cita_obj:
                    return redirect('mis_citas_dentista')
                elif paciente_id_post:
...
def editar_odontograma(request, odontograma_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            # Registrar auditoría - Editar odontograma
            registrar_auditoria(
                usuario=perfil,
                accion='editar',
                modulo='odontogramas',
                descripcion=f'Ficha odontológica editada para {odontograma.paciente_nombre}',
                detalles=f'Odontograma ID: {odontograma.id}, Paciente: {odontograma.paciente_nombre}',
                objeto_id=odontograma.id,
                tipo_objeto='Odontograma',
                request=request
            )
...
def eliminar_odontograma(request, odontograma_id):
    ...
    if request.method == 'POST':
        ...
        odontograma_id_val = odontograma.id
        try:
            # Registrar auditoría - Eliminar odontograma (antes de eliminar)
            registrar_auditoria(
                usuario=perfil,
                accion='eliminar',
                modulo='odontogramas',
                descripcion=f'Ficha odontológica eliminada para {paciente_nombre}',
                detalles=f'Odontograma ID: {odontograma_id_val}, Paciente: {paciente_nombre}',
                objeto_id=odontograma_id_val,
                tipo_objeto='Odontograma',
                request=request
            )
...
def exportar_odontograma_pdf(request, odontograma_id):
    ...
    # Registrar auditoría - Exportar odontograma a PDF (solo para dentistas)
    if perfil.es_dentista():
        registrar_auditoria(
            usuario=perfil,
            accion='exportar',
            modulo='odontogramas',
            descripcion=f'Ficha odontológica exportada a PDF para {odontograma.paciente_nombre}',
            detalles=f'Odontograma ID: {odontograma.id}, Paciente: {odontograma.paciente_nombre}',
            objeto_id=odontograma.id,
            tipo_objeto='Odontograma',
            request=request
...
def gestor_documentos(request):
    ...
    # Aplicar filtros
    if tipo_filtro:
        if tipo_filtro == 'odontograma':
            # Para fichas odontológicas, también incluir odontogramas sin documento asociado
            # Crear documentos automáticamente para odontogramas que no tienen documento
            from historial_clinico.models import Odontograma
            odontogramas_sin_documento = Odontograma.objects.exclude(
                id__in=DocumentoCliente.objects.filter(tipo='odontograma', odontograma__isnull=False).values_list('odontograma_id', flat=True)
            ).select_related('cliente', 'dentista')
            
            # Crear documentos para odontogramas que no tienen documento asociado
            for odontograma in odontogramas_sin_documento:
                try:
                    cliente_doc = odontograma.cliente
                    if not cliente_doc and odontograma.paciente_email:
                        try:
                    ...
                    DocumentoCliente.objects.get_or_create(
                        ...
                        defaults={
                            ...
                            'titulo': f'Ficha Odontológica - {odontograma.paciente_nombre}',
                            'descripcion': f'Ficha odontológica del paciente {odontograma.paciente_nombre}',
                            'generado_por': odontograma.dentista,
                            'fecha_generacion': odontograma.fecha_creacion,
                        }
                    )
                except Exception as e:
                    logger.error(f"Error al crear documento automático para odontograma {odontograma.id}: {str(e)}")
            
            # Ahora filtrar solo por tipo odontograma
            documentos = documentos.filter(tipo='odontograma')
        elif tipo_filtro == 'presupuesto':
            # Para presupuestos/tratamientos, también incluir planes sin documento asociado
            # Crear documentos automáticamente para planes que no tienen documento
            from historial_clinico.models import PlanTratamiento
            planes_sin_documento = PlanTratamiento.objects.exclude(
                id__in=DocumentoCliente.objects.filter(tipo='presupuesto', plan_tratamiento__isnull=False).values_list('plan_tratamiento_id', flat=True)
            ).select_related('cliente', 'creado_por')
            
            # Crear documentos para planes que no tienen documento asociado
            for plan in planes_sin_documento:
                try:
                    DocumentoCliente.objects.get_or_create(
                        plan_tratamiento=plan,
                        tipo='presupuesto',
                        defaults={
                            ...
                            'fecha_generacion': plan.creado_el if hasattr(plan, 'creado_el') else timezone.now(),
                        }
                    )
                except Exception as e:
                    logger.error(f"Error al crear documento automático para plan {plan.id}: {str(e)}")
            
            # Ahora filtrar solo por tipo presupuesto
            documentos = documentos.filter(tipo='presupuesto')
        else:
    ...
    tipos_excluidos = ['receta', 'factura', 'nota_medica', 'otro', 'radiografia']
...
def crear_consentimiento(request):
    ...
    if request.method == 'POST':
        try:
            ...
            # Si se está creando para un plan de tratamiento, verificar que no tenga ya un consentimiento
            if plan_id:
                try:
                    plan = PlanTratamiento.objects.get(id=plan_id)
                    if plan.consentimientos.exists():
                        messages.error(request, 'Este plan de tratamiento ya tiene un consentimiento informado. Solo se permite un consentimiento por plan.')
                        return redirect('gestor_consentimientos')
...
def crear_consentimiento_desde_plan(request, plan_id):
    ...
    # Verificar si ya existe un consentimiento para este plan
    consentimiento_existente = plan.consentimientos.exists()
    ...
    if request.method == 'POST':
        # Verificar nuevamente antes de crear (por si acaso se intenta crear desde otro lugar)
        if plan.consentimientos.exists():
            messages.error(request, 'Este plan de tratamiento ya tiene un consentimiento informado. Solo se permite un consentimiento por plan.')
            return redirect('detalle_plan_tratamiento', plan_id=plan.id)
        try:
...
def firmar_consentimiento(request, consentimiento_id):
    ...
    try:
        ...
        # Si el consentimiento está asociado a un plan de tratamiento, verificar si se puede activar
        mensaje = 'Consentimiento firmado exitosamente.'
        if consentimiento.plan_tratamiento:
            plan = consentimiento.plan_tratamiento
            # Si ya tiene presupuesto aceptado y está en 'pendiente_aprobacion', activar automáticamente
            if plan.presupuesto_aceptado and plan.estado == 'pendiente_aprobacion':
                plan.estado = 'en_progreso'
                plan.fecha_aprobacion = timezone.now()
                plan.save()
                mensaje = 'Consentimiento firmado. El tratamiento se ha activado automáticamente (presupuesto ya estaba aceptado).'
...
def firmar_consentimiento_recepcion(request, plan_id, consentimiento_id):
    ...
    try:
        ...
        # Si el consentimiento está asociado a un plan de tratamiento, verificar si se puede activar
        mensaje = f'El documento firmado del consentimiento "{consentimiento.titulo}" fue subido exitosamente.'
        if consentimiento.plan_tratamiento:
            plan = consentimiento.plan_tratamiento
            # Si ya tiene presupuesto aceptado y está en 'pendiente_aprobacion', activar automáticamente
            if plan.presupuesto_aceptado and plan.estado == 'pendiente_aprobacion':
                ...
                plan.fecha_aprobacion = timezone.now()
                plan.save()
                mensaje = f'Consentimiento subido. El tratamiento se ha activado automáticamente (presupuesto ya estaba aceptado).'
...
# ============================================================================
def _calcular_monto_cita(cita):
    """Función auxiliar para calcular el monto de una cita"""
    if cita.precio_cobrado:
        return float(cita.precio_cobrado)
    elif cita.tipo_servicio and cita.tipo_servicio.precio_base:
        return float(cita.tipo_servicio.precio_base)
    return 0


def _calcular_ingresos_citas(citas_queryset):
    """Función auxiliar para calcular ingresos totales de un queryset de citas"""
    total = 0
    for cita in citas_queryset:
        total += _calcular_monto_cita(cita)
    return total
...
def gestor_finanzas(request):
    ...
    try:
        ...
        logger.error(f"Error al verificar permisos en gestor_finanzas: {str(e)}")
        messages.error(request, 'Error al acceder a las finanzas.')
        return redirect('panel_trabajador')
    ...
    try:
        ...
        citas_disponibles = Cita.objects.filter(estado='disponible').count()
        citas_reservadas = Cita.objects.filter(estado='reservada').count()
        citas_completadas_count = Cita.objects.filter(estado='completada').count()
        citas_canceladas = Cita.objects.filter(estado='cancelada').count()
        citas_hoy = Cita.objects.filter(fecha_hora__date=hoy).count()
        
        # Estadísticas financieras generales - optimizado
        # Calcular ingresos considerando precio_cobrado o precio del servicio
        total_ingresos = _calcular_ingresos_citas(citas_completadas)
        
        # Sumar ingresos manuales (optimizado con agregación)
        ...
        ingresos_mes = _calcular_ingresos_citas(citas_completadas_mes)
        
        # Sumar ingresos manuales del mes (optimizado)
        ...
        ingresos_año = _calcular_ingresos_citas(citas_completadas_año)
        
        # Sumar ingresos manuales del año (optimizado)
        ingresos_año += IngresoManual.objects.filter(
        ...
        # Ingresos por servicio (optimizado - una sola iteración)
        ...
        for cita in citas_completadas:
            ...
            ingresos_por_servicio_dict[servicio_nombre]['total_ingresos'] += _calcular_monto_cita(cita)
        ...
        for i in range(11, -1, -1):
            ...
            ingresos_mes_actual = _calcular_ingresos_citas(citas_mes_actual)
        ...
        # Ingresos por dentista (optimizado)
        ...
        for cita in citas_completadas.filter(dentista__isnull=False):
            ...
            ingresos_por_dentista_dict[dentista_nombre]['total_ingresos'] += _calcular_monto_cita(cita)
        ...
        for cita in citas_completadas:
            ...
            if cita.precio_cobrado is not None or (cita.tipo_servicio and cita.tipo_servicio.precio_base):
                monto_cita = _calcular_monto_cita(cita)
        ...
        # ===== EGRESOS (Gastos) =====
        # Los egresos ahora son SOLO manuales o los generados al recibir solicitudes de proveedores
        # NO se calculan egresos automáticos basados en movimientos de insumos tipo "entrada"
        # egresos_manuales ya está definido fuera del try
        
        # Calcular egresos totales (solo manuales)
        total_egresos = EgresoManual.objects.aggregate(Sum('monto'))['monto__sum'] or 0
        
        # Egresos del mes (solo manuales)
        egresos_mes = EgresoManual.objects.filter(
            fecha__gte=inicio_mes,
            fecha__lte=fin_mes
        ).aggregate(Sum('monto'))['monto__sum'] or 0
        
        # Egresos del año (solo manuales)
        egresos_año = EgresoManual.objects.filter(
            fecha__gte=inicio_año
        ).aggregate(Sum('monto'))['monto__sum'] or 0
        
        # Egresos por mes (últimos 12 meses) - solo manuales
        egresos_por_mes = []
        from calendar import monthrange
        for i in range(11, -1, -1):
            mes_fecha = hoy - timedelta(days=30 * i)
            ...
            # Sumar solo egresos manuales del mes
            egresos_mes_actual = EgresoManual.objects.filter(
            ...
            egresos_por_mes.append({
                ...
                'movimientos': EgresoManual.objects.filter(
                    fecha__gte=mes_inicio,
                    fecha__lte=mes_fin
                ).count()
            })
        
        # Todos los egresos combinados para la tabla (solo manuales)
        todos_egresos_list = []
        
        # Agregar solo egresos manuales
        ...
        # Ordenar por fecha (más recientes primero) - solo egresos manuales
        def get_fecha(item):
            from datetime import datetime
            # Crear datetime aware para que sea compatible con los otros
        ...
        import traceback
        logger.error(f"Error al calcular estadísticas financieras: {str(e)}\n{traceback.format_exc()}")
        messages.error(request, f'Error al cargar las estadísticas financieras: {str(e)}')
        # No redirigir, mostrar la página con valores por defecto para que el usuario vea el error
...
def toggle_estado_cliente(request, cliente_id):
    ...
    try:
        ...
        try:
            ...
            user = User.objects.filter(email=cliente.email).first()
            if user:
                # Desactivar/activar también el usuario de Django
            ...
            # Si hay algún error al manejar el usuario, continuar pero loguear el error
            logger.error(f"Error al actualizar usuario web para cliente {cliente_id}: {e}", exc_info=True)
            mensaje = f'Cliente {"activado" if nuevo_estado else "desactivado"} exitosamente.'
            # Nota: No fallamos la operación principal si solo falla la actualización del usuario web
        ...
        import logging
        logger = logging.getLogger(__name__)
        logger.error(f"Error inesperado al cambiar estado del cliente {cliente_id}: {e}", exc_info=True)
        return JsonResponse({
            'success': False,
            'error': f'Error al cambiar el estado del cliente: {str(e)}'
        }, status=500)
...
def agregar_radiografia(request, paciente_id):
    ...
    if request.method == 'POST':
        try:
            ...
            # Registrar auditoría - Agregar radiografía
            registrar_auditoria(
                usuario=perfil,
                accion='crear',
                modulo='radiografias',
                descripcion=f'Radiografía agregada para {paciente["nombre_completo"]}',
                detalles=f'Tipo: {dict(Radiografia.TIPO_RADIOGRAFIA_CHOICES).get(tipo, tipo)}, Paciente: {paciente["nombre_completo"]}, Descripción: {descripcion[:100] if descripcion else "N/A"}',
                objeto_id=radiografia.id,
                tipo_objeto='Radiografia',
                request=request
            )
...
def editar_radiografia(request, radiografia_id):
    ...
    if request.method == 'POST':
        try:
            ...
            tipo_anterior = radiografia.tipo
            descripcion_anterior = radiografia.descripcion
            ...
            # Registrar auditoría - Editar radiografía
            registrar_auditoria(
                usuario=perfil,
                accion='editar',
                modulo='radiografias',
                descripcion=f'Radiografía editada para {radiografia.paciente_nombre}',
                detalles=f'Radiografía ID: {radiografia.id}, Tipo: {dict(Radiografia.TIPO_RADIOGRAFIA_CHOICES).get(tipo, tipo)}, Paciente: {radiografia.paciente_nombre}',
                objeto_id=radiografia.id,
                tipo_objeto='Radiografia',
                request=request
            )
...
def eliminar_radiografia(request, radiografia_id):
    ...
    try:
        radiografia = Radiografia.objects.get(id=radiografia_id, dentista=perfil)
        paciente_email = radiografia.paciente_email
        paciente_nombre = radiografia.paciente_nombre
        tipo_radiografia = radiografia.tipo
        ...
        # Registrar auditoría - Eliminar radiografía (antes de eliminar)
        registrar_auditoria(
            usuario=perfil,
            accion='eliminar',
            modulo='radiografias',
            descripcion=f'Radiografía eliminada para {paciente_nombre}',
            detalles=f'Radiografía ID: {radiografia_id}, Tipo: {dict(Radiografia.TIPO_RADIOGRAFIA_CHOICES).get(tipo_radiografia, tipo_radiografia)}, Paciente: {paciente_nombre}',
            objeto_id=radiografia_id,
            tipo_objeto='Radiografia',
            request=request
        )
...
def perfil_cliente(request, cliente_id):
    ...
    estadisticas = {
        ...
        'citas_no_asistidas': citas.filter(estado='completada', precio_cobrado=0).count(),
...
def gestor_servicios(request):
    ...
    # Consumir todos los mensajes antiguos para evitar que se muestren alertas de otras páginas
    # Esto asegura que solo se muestren mensajes generados en esta vista
...
def crear_servicio(request):
    ...
    if request.method == 'POST':
        try:
            ...
            requiere_dentista = True  # Valor por defecto
    ...
    if request.method == 'POST':
        ...
        if citas_asociadas > 0:
            messages.error(
                ...
                f'Debe eliminar o cambiar el servicio de todas las citas asociadas antes de poder eliminar este servicio.'
            )
            return redirect('gestor_servicios')
...
def listar_planes_tratamiento(request):
    ...
    # Obtener planes base para estadísticas (antes de aplicar filtros)
    planes_base = planes
    
    # Contadores por estado para el menú lateral
    contadores_estado = {
        'todos': planes_base.count(),
        'borrador': planes_base.filter(estado='borrador').count(),
        'pendiente_aprobacion': planes_base.filter(estado='pendiente_aprobacion').count(),
        'aprobado': planes_base.filter(estado='aprobado').count(),
        'en_progreso': planes_base.filter(estado='en_progreso').count(),
        'completado': planes_base.filter(estado='completado').count(),
        'cancelado': planes_base.filter(estado='cancelado').count(),
        'rechazado': planes_base.filter(estado='rechazado').count(),
    }
    
    # Determinar sección activa para el menú lateral
    seccion_activa = 'todos'
    if estado_filtro:
        seccion_activa = estado_filtro
...
def crear_plan_tratamiento(request):
    ...
    if request.method == 'POST':
        cliente_id = request.POST.get('cliente_id', '').strip()
        dentista_id = request.POST.get('dentista_id', '').strip()
        ...
        if not dentista_id or not dentista_id.isdigit():
            messages.error(request, 'Debe seleccionar un dentista válido.')
            return redirect('crear_plan_tratamiento')
        ...
        # Validar que el cliente no tenga un tratamiento activo
        # Incluimos 'borrador' por si hay planes antiguos, pero el nuevo flujo no crea 'borrador'
        planes_activos = PlanTratamiento.objects.filter(
            cliente=cliente
        ).exclude(estado__in=['completado', 'cancelado', 'rechazado'])
        
        if planes_activos.exists():
            plan_activo = planes_activos.first()
            messages.error(
                request, 
                f'El cliente {cliente.nombre_completo} ya tiene un tratamiento activo: "{plan_activo.nombre}" '
                f'(Estado: {plan_activo.get_estado_display()}). '
                'Debe completar o cancelar el tratamiento actual antes de crear uno nuevo.'
            )
            return redirect('crear_plan_tratamiento')
        
        # Obtener odontograma inicial si se proporciona
        odontograma_id = request.POST.get('odontograma_inicial_id', '').strip()
        odontograma_inicial = None
        if odontograma_id and odontograma_id.isdigit():
            try:
                odontograma_inicial = Odontograma.objects.get(id=int(odontograma_id))
            except (Odontograma.DoesNotExist, ValueError):
                pass
        ...
        try:
            ...
            if not diagnostico:
                messages.error(request, 'El diagnóstico es requerido.')
                return redirect('crear_plan_tratamiento')
            
            if not objetivo:
                messages.error(request, 'El objetivo es requerido.')
                return redirect('crear_plan_tratamiento')
            ...
            if not presupuesto_str or presupuesto_str == '0':
                messages.error(request, 'El presupuesto total es requerido y debe ser mayor a 0.')
                return redirect('crear_plan_tratamiento')
            
            try:
                presupuesto_total = float(presupuesto_str)
            except (ValueError, TypeError):
                messages.error(request, 'El presupuesto total debe ser un número válido.')
                return redirect('crear_plan_tratamiento')
            
            # Validar que el presupuesto sea mayor a 0
            if presupuesto_total <= 0:
                messages.error(request, 'El presupuesto total debe ser mayor a 0.')
                return redirect('crear_plan_tratamiento')
            
            # Sin descuento, el precio final es igual al presupuesto total
            precio_final = presupuesto_total
            ...
            # El estado se maneja automáticamente, siempre inicia como 'pendiente_aprobacion'
            plan = PlanTratamiento.objects.create(
                ...
                precio_final=precio_final,
                estado='pendiente_aprobacion',  # Estado automático, siempre inicia como pendiente_aprobacion
                ...
                notas_internas='',  # Sin notas internas
            ...
            registrar_auditoria(
                ...
                detalles=f'Cliente: {cliente.nombre_completo}, Dentista: {dentista.nombre_completo}, Presupuesto: ${presupuesto_total:,.0f}',
                objeto_id=plan.id,
            ...
            try:
                DocumentoCliente.objects.get_or_create(
                    ...
                    defaults={
                        'cliente': cliente,
                        'titulo': f'Presupuesto - {nombre}',
                        'descripcion': f'Presupuesto del tratamiento {nombre}',
                        'generado_por': perfil,
                        'fecha_generacion': timezone.now(),
                    }
                )
            except Exception as e:
                logger.error(f"Error al crear documento de presupuesto: {str(e)}")
...
def detalle_plan_tratamiento(request, plan_id):
    ...
    if perfil.es_administrativo() or (perfil.es_dentista() and plan.dentista == perfil):
        # Verificar condiciones para aprobar (solo si está en pendiente_aprobacion)
        puede_aprobar = tiene_consentimiento_firmado and presupuesto_aceptado and plan.estado == 'pendiente_aprobacion'
    
    # Verificar si puede finalizar el tratamiento
    puede_finalizar = False
    if perfil.es_administrativo() or (perfil.es_dentista() and plan.dentista == perfil):
        # Solo se puede finalizar si está en progreso
        puede_finalizar = plan.estado == 'en_progreso'
    ...
    if perfil.es_administrativo():
        puede_crear_citas = tiene_consentimiento_firmado and presupuesto_aceptado and plan.estado == 'en_progreso'
...
def aceptar_presupuesto_tratamiento(request, plan_id):
    ...
    # Si ya tiene consentimiento firmado y está en 'pendiente_aprobacion', activar automáticamente
    tiene_consentimiento_firmado = plan.consentimientos.filter(estado='firmado').exists()
    if tiene_consentimiento_firmado and plan.estado == 'pendiente_aprobacion':
        plan.estado = 'en_progreso'
        plan.fecha_aprobacion = timezone.now()
    
    plan.save()
    
    mensaje = 'Presupuesto marcado como aceptado correctamente.'
    if tiene_consentimiento_firmado and plan.estado == 'en_progreso':
        mensaje = 'Presupuesto aceptado. El tratamiento se ha activado automáticamente (consentimiento ya estaba firmado).'
...
def aprobar_tratamiento(request, plan_id):
    ...
    if plan.estado not in ['pendiente_aprobacion']:
        return JsonResponse({
                'success': False,
    ...
    # Aprobar el tratamiento - cambia a 'en_progreso' (activo)
    plan.estado = 'en_progreso'
    plan.fecha_aprobacion = timezone.now()
    plan.save()
    ...
    return JsonResponse({
        'success': True,
        'message': 'Tratamiento activado correctamente. El tratamiento está ahora en progreso.'
    })
...
def finalizar_tratamiento(request, plan_id):
    ...
    if plan.estado != 'en_progreso':
        return JsonResponse({
                'success': False,
                'error': f'No se puede finalizar un tratamiento en estado "{plan.get_estado_display()}". Solo se pueden finalizar tratamientos en progreso.'
            }, status=400)
...
def crear_cita_desde_plan(request, plan_id):
    ...
    if request.method == 'POST':
        ...
        if plan.estado != 'en_progreso':
            # Compatibilidad: si está en 'aprobado' (planes antiguos), cambiarlo a 'en_progreso'
            if plan.estado == 'aprobado':
                ...
                return JsonResponse({
                    'success': False, 
                    'error': f'No se pueden crear citas para un tratamiento en estado "{plan.get_estado_display()}". El tratamiento debe estar en progreso.'
                }, status=400)
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            try:
                ...
                resultado = enviar_notificaciones_cita(cita)
                if resultado.get('email', {}).get('enviado'):
                    logger.info(f"Correo de confirmación enviado exitosamente para cita {cita.id} del plan {plan.id}")
                else:
                    error_email = resultado.get('email', {}).get('error', 'Error desconocido')
                    logger.warning(f"No se pudo enviar correo de confirmación para cita {cita.id}: {error_email}")
            except Exception as e:
...
def editar_plan_tratamiento(request, plan_id):
    """Edita un plan de tratamiento existente - Solo si está en pendiente_aprobacion"""
    try:
        perfil = Perfil.objects.get(user=request.user)
        if not perfil.activo:
            messages.error(request, 'Tu cuenta está desactivada.')
    ...
    if not perfil.es_administrativo():
        messages.error(request, 'Solo los administrativos pueden editar planes de tratamiento.')
    ...
    if not plan.puede_ser_editado_por(perfil):
        messages.error(request, f'No se puede editar un tratamiento en estado "{plan.get_estado_display()}". Solo se pueden editar tratamientos pendientes de aprobación.')
        return redirect('detalle_plan_tratamiento', plan_id=plan.id)
    ...
    if request.method == 'POST':
        ...
        if not nombre:
            messages.error(request, 'El nombre del plan es requerido.')
            return redirect('editar_plan_tratamiento', plan_id=plan.id)
        ...
        try:
            ...
            plan.diagnostico = diagnostico
            plan.objetivo = objetivo
            plan.presupuesto_total = presupuesto_total
            plan.precio_final = presupuesto_total  # Sin descuento
            plan.descuento = 0
            
            # Si se cambió el presupuesto y ya estaba aceptado, resetear la aceptación
            if plan.presupuesto_aceptado and presupuesto_total != plan.presupuesto_total:
                plan.presupuesto_aceptado = False
                plan.fecha_aceptacion_presupuesto = None
            
            citas_estimadas = request.POST.get('citas_estimadas', '').strip()
            plan.citas_estimadas = int(citas_estimadas) if citas_estimadas and citas_estimadas.isdigit() else None
            
            fecha_inicio = request.POST.get('fecha_inicio_estimada', '').strip()
            plan.fecha_inicio_estimada = fecha_inicio if fecha_inicio else None
            
            fecha_fin = request.POST.get('fecha_fin_estimada', '').strip()
            plan.fecha_fin_estimada = fecha_fin if fecha_fin else None
            
            plan.notas_paciente = request.POST.get('notas_paciente', '')
            plan.save()
            ...
            registrar_auditoria(
                ...
                accion='editar',
                modulo='planes_tratamiento',
                descripcion=f'Plan de tratamiento editado: {nombre}',
                detalles=f'Cliente: {plan.cliente.nombre_completo}, Dentista: {plan.dentista.nombre_completo}, Presupuesto: ${presupuesto_total:,.0f}',
                objeto_id=plan.id,
                tipo_objeto='PlanTratamiento',
                request=request
            )
            
            messages.success(request, f'Plan de tratamiento "{plan.nombre}" actualizado exitosamente.')
            return redirect('detalle_plan_tratamiento', plan_id=plan.id)
            
        except Exception as e:
            messages.error(request, f'Error al actualizar el plan: {str(e)}')
            logger.error(f"Error al editar plan de tratamiento: {e}")
            return redirect('editar_plan_tratamiento', plan_id=plan.id)
    ...
    return render(request, 'citas/planes_tratamiento/editar_plan_tratamiento.html', context)

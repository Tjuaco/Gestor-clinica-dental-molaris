...
def verificar_telefono(request):
    ...
    email = registro_data.get('email', '')  # Obtener email de la sesión
...
def reenviar_codigo(request):
    ...
    if request.method == 'POST' and 'registro_data' in request.session:
        ...
        try:
            ...
            codigo_obj = CodigoVerificacion.generar_codigo(telefono_normalizado)
            # Enviar por email
            email = registro_data.get('email', '')
            if email:
                enviar_codigo_por_email(email, codigo_obj.codigo)
                return JsonResponse({
                    'success': True,
                    'message': f'Código reenviado por email a {email}'
                })
            else:
                return JsonResponse({
                    'success': False,
                    'message': 'No se encontró email en la sesión'
                })
        except Exception as e:
            import logging
            logger = logging.getLogger(__name__)
